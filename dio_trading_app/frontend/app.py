...
# If a cloud URL is configured, skip localhost discovery
_IS_CLOUD = not DEFAULT_API_BASE.startswith("http://localhost") and not DEFAULT_API_BASE.startswith("http://127.")
API_DISCOVERY_PORTS = () if _IS_CLOUD else (8000, 8001, 8002, 8003, 8004, 8005)
...
def _render_signal_card(pair_name: str, item: dict, expanded: bool = False) -> None:
    ...
    dir_icon = _dir_emoji(direction)

    # SL / TP distances from entry
    sl_dist = _pip_label(pair_name, sl_p - entry_p) if entry_p and sl_p else "—"
    tp_dist = _pip_label(pair_name, tp_p - entry_p) if entry_p and tp_p else "—"

    rank = item.get("rank")
    outcome_tag = item.get("status", "")
    is_tp = "TP HIT" in outcome_tag
    is_sl = "SL HIT" in outcome_tag

    if rank:
        badge = "🥇" if rank == 1 else "🥈" if rank == 2 else "🥉" if rank == 3 else f"#{rank}"
        header = f"{badge} {dir_icon} {direction} [{item['tf']}] @ {_pfmt(pair_name, entry_p)} | Score {item.get('rank_score', 0)} | {item.get('time', '—')}"
    elif is_tp:
        header = f"🏆 TP HIT | {dir_icon} {direction} [{item['tf']}] @ {_pfmt(pair_name, entry_p)} | {item.get('time', '—')}"
    elif is_sl:
        header = f"🛑 SL HIT | {dir_icon} {direction} [{item['tf']}] @ {_pfmt(pair_name, entry_p)} | {item.get('time', '—')}"
    else:
        header = f"{dir_icon} {direction} [{item['tf']}] @ {_pfmt(pair_name, entry_p)} | {outcome_tag} | {item.get('time', '—')}"

    with st.expander(header, expanded=expanded):
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Entry", _pfmt(pair_name, entry_p))
        c2.metric("🔴 SL", _pfmt(pair_name, sl_p), delta=sl_dist, delta_color="inverse")
        c3.metric("🟢 TP", _pfmt(pair_name, tp_p), delta=tp_dist)
...
def _render_three_types(pair_name: str, buckets: dict) -> None:
    ...
    # ── 1) Running ────────────────────────────────────────────
    st.markdown("### 🟢 Running — Entry active now")
    if not buckets.get("running"):
        st.caption("কোনো active signal নেই।  পরবর্তী scan-এ চেক করুন।")
    for item in buckets.get("running", []):
        ...
        st.markdown(
            f"<div style='padding:6px 10px;border-left:4px solid #22c55e;background:#052e16;border-radius:6px;margin-bottom:4px;'>"
            f"{badge} {dir_icon} <b>{direction}</b> &nbsp;|&nbsp; <b>[{item['tf']}]</b> &nbsp;|&nbsp; "
            f"Conf <b>{conf:.0f}</b> &nbsp;|&nbsp; ML <b>{ml:.0f}%</b> &nbsp;|&nbsp; {item.get('time', '—')}"
            f"</div>",
            unsafe_allow_html=True,
        )
    ...
    # ── 2) Upcoming ───────────────────────────────────────────
    st.markdown("### 🟣 Upcoming — Prepare entry")
    if not buckets.get("upcoming"):
        st.caption("সব timeframe মনিটর হচ্ছে।")
    for item in buckets.get("upcoming", []):
        expl = item.get("explanation", "")
        # Try to pick direction from explanation for the emoji
        up_dir = "BUY" if expl.startswith("BUY") else "SELL" if expl.startswith("SELL") else ""
        up_icon = _dir_emoji(up_dir) if up_dir else "🟣"
        status = item.get("status", "UPCOMING")
        is_likely = "likely" in status.lower()
        hint_color = "#7c3aed" if is_likely else "#4b5563"
        border_color = "#a78bfa" if is_likely else "#6b7280"
        st.markdown(
            f"<div style='padding:6px 10px;border-left:4px solid {border_color};background:#1e1b4b20;border-radius:6px;margin-bottom:4px;'>"
            f"{up_icon} <b>[{item['tf']}]</b> &nbsp;|&nbsp; {status} &nbsp;|&nbsp; {item.get('time', '—')}"
            f"</div>",
            unsafe_allow_html=True,
        )
        if expl:
    ...
    # ── 3) Passed ─────────────────────────────────────────────
    st.markdown("### ⬛ Passed — Closed trades")
    if not buckets.get("passed"):
        st.caption("কোনো closed signal নেই।")
    for item in buckets.get("passed", []):
        sig = item.get("signal") or {}
        direction = str(sig.get("direction", "")).upper()
        dir_icon = _dir_emoji(direction)
        status = item.get("status", "")
        is_tp = "TP HIT" in status
        is_sl = "SL HIT" in status
        rr = float(sig.get("risk_reward", 2) or 2) if sig else 2.0
        if is_tp:
            result_badge = f"🏆 TP HIT +{rr:.1f}R"
            border = "#22c55e"
            bg = "#052e16"
        elif is_sl:
            result_badge = "🛑 SL HIT  -1R"
            border = "#ef4444"
            bg = "#2a0000"
        else:
            result_badge = f"⬛ {status}"
            border = "#6b7280"
            bg = "#111827"
        st.markdown(
            f"<div style='padding:6px 10px;border-left:4px solid {border};background:{bg};border-radius:6px;margin-bottom:4px;'>"
            f"{result_badge} &nbsp;|&nbsp; {dir_icon} <b>{direction}</b> &nbsp;|&nbsp; <b>[{item['tf']}]</b> &nbsp;|&nbsp; {item.get('time', '—')}"
            f"</div>",
            unsafe_allow_html=True,
        )
        if item.get("signal"):
